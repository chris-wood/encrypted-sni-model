...
\author{Karthikeyan Bhargavan, Eric Rescorla, Christopher A. Wood}
\date{\today}
...
\subsection{Existing Design Overview and Known Attacks} \label{sec:known-attacks}
...
%% TODO(caw): include cut-and-paste attack
...
\resizebox{0.8\columnwidth}{!}{\begin{msc}{}
  ...
  \nextlevel[2]
  \nextlevel
  \mess{\emph{Client Flight} ($S, g^x, \mathsf{HPKESeal}(\ksni, \sni)$)}{cl}{sr}
  \nextlevel[2]
  \mess{\emph{Server Flight} ($g^y,\mathsf{Enc}(g^{xy}, \cert, ...), $)}{sr}{cl}
  \nextlevel
...
\end{center}
\caption{Client reaction attack.}
...
that the server processed the SNI. Figure \ref{fig:draft4} shows the ESNI design in draft-04 of the protocol. 
...
\resizebox{0.8\columnwidth}{!}{\begin{msc}{}
  ...
  \nextlevel[2]
  \nextlevel
  \mess{\emph{Client Flight} ($S, g^x, \mathsf{HPKESeal}(K_s, (r, \sni))$)}{cl}{sr}
  \nextlevel[2]
...
\end{center}
\caption{draft-ietf-tls-esni-04 protocol (without HRR).}
\label{fig:draft4}
\end{figure}
...
the certificate. This attack flow is shown in Figure \ref{fig:hrr-attack}.
...
\begin{figure}[th!]
 ...
 \resizebox{0.8\columnwidth}{!}{\begin{msc}{}
   ...
   \setlength{\instwidth}{1.75cm}
   \declinst{cl}{}{Client $C$}
   \declinst{at}{}{Attacker $A$}
   \declinst{sr}{}{Server $S$}
   \action*{$\mathit{Knows}\ \sni, \ksni$}{cl}
   \action*{$\mathit{Knows}\ \ksnipriv$}{sr}
   \nextlevel[2]
   \nextlevel
   \mess{\emph{ClientHello1:} ($g^x, \mathsf{HPKESeal}(\ksni, (\sni, nonce), g^x)$)}{cl}{sr}
   \nextlevel[2]
   \mess{\emph{HRR:} (...)}{sr}{cl}
   \nextlevel[2]
   \mess{\emph{ClientHello2:} ($g^{x'}, \mathsf{HPKESeal}(\ksni, (\sni, nonce), g^{x'})$)}{cl}{at}
   \nextlevel[2]
   \mess{\emph{ClientHello2:} ($g^{a}, \mathsf{HPKESeal}(\ksni, (dummysni, dummynonce), g^{a})$)}{at}{sr}
   \nextlevel    
   \action*{\laction{Derive Keys:}{
      ss = \dh{g^a}{g^y} \\
      hk = \kdf{ss} \\
...
\end{center}
\caption{Active HRR attack.}
\label{fig:hrr-attack}
\end{figure}
...
binds in $g^x$. However, if the server uses the SNI from first ClientHello and then
the key share from the second ClientHello, which is attacker-controlled, then the 
attacker is free to generate their own ESNI extension containing a bogus ESNI 
and nonce. The server correctly verifies that these correspond to $g^a$ but because
these are all supplied by the attacker, this check succeeds and the server encrypts 
the certificate to the attacker with a key known to the attacker.
...
client SNIs. This attack, which is shown in Figure \ref{fig:ticket-attack}, is also a mix-and-match attack.
...
\end{center}
\label{fig:ticket-attack}
\end{figure}

This is a failure of binding: currently, ESNI is not bound enough of the ClientHello. (All of the proposals described
in Section \ref{sec:proposals} prevent this attack in the same way: they bind the ESNI to the entire ClientHello. 
If the attacker attaches their own PSK, then this invalidates the binding, thus preventing the mix-and-match. None of 
this depends on the confidentiality of the ClientHello.)

\section{Core Protocol} \label{sec:core} % Basic anonymous DH model (handshake secret integrity)

\todo[inline]{This is still work in progress undergoing refinements.}
...
handshake. In this section, we describe an ideal ESNI protocol that addresses these shortcomings.
...
\section{Attack Flow Discussion}
\subsection{HRR Attack}

With the HRR attack from \label{sec:known-attacks}, we now examine why each of the variants
described above prevents the attack. In each case, the attacker is unable to determine the handshake keys and
